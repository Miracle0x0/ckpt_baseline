...
# except ImportError:
#     from bert.file_utils import cached_path
...
class NextSentencePrediction(nn.Module):
    ...
    def forward(self, x, targets):
        x_ = self.softmax(x)
        targets_ = F.one_hot(targets, num_classes=2)
        # loss = self.criterion(x_, targets)
        loss_ = self.criterion_(x, targets_.float())
        correct = x_.argmax(dim=-1).eq(targets).sum().item()
        return loss_, correct / 32
