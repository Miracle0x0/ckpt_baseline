...
import time
import enum
from torch.multiprocessing import Pool, Process, set_start_method, Manager, Value, Lock, freeze_support
...
class CFManager:
    ...
    def __init__(
        ...
        chk_dir: str,
        chk,
        keep_epoch_chk: bool = True,
        overwrite: bool = True,
        mode=CFMode.AUTO,
        chk_prefix: str = "model_v_",
    ):
        """
        ...
        `chk_prefix` : Prefix for the checkpoint file
        """
        ...
        if self.overwrite and not self.keep_epoch_chk:
    ...
    def save(
        ...
        * `is_epoch` : True if this is epoch boundary
        ...
        self.chk_global_id += 1
        chk_fname = f"{self.chk_prefix}{self.chk_global_id}"
        filepath = self._get_full_path(chk_fname)
        if is_epoch:
            chk_fname_link = f"{self.chk_prefix}{self.chk_global_id}_{epoch}"
        ...
        if not is_epoch:
            keywords = {"iter_chk": self.available_chk_iters, "overwrite": self.overwrite}
            self.chk_process = fn(
                target=self.chk._serialize_and_persist,
                args=[filepath, self.chk.latest_snapshot, self.active_snapshot, self.lock],
                kwargs=keywords,
            )
        else:
            keywords = {
    ...
    def save_cpu(
        ...
        print("Function is {}".format(fn))
        if not is_epoch:
            keywords = {"iter_chk": self.available_chk_iters, "overwrite": self.overwrite, "profile": profile_snap}
            self.chk_process = fn(
                target=self.chk._snapshot_and_persist_async,
                args=[filepath, self.active_snapshot, self.in_progress_snapshot, self.lock, snap_ptr],
                kwargs=keywords,
            )
        else:
            keywords = {
    ...
    def restore(self, latest: bool = True, epoch: int = 0, gpu=0):
        """
    ...
    def initalize_chk_dir(self):
        if os.path.exists(self.chk_dir):
            ...
            chk_files = [
                ...
                if os.path.isfile(os.path.join(self.chk_dir, f))
            ]
            ...
            if os.path.exists(epoch_chk_dir):
                epoch_chk_files = [
                    ...
                    if os.path.isfile(os.path.join(epoch_chk_dir, f))
                ]
    ...
    def get_latest_checkpoint(self, latest: bool = True, epoch: int = 0):
        """
    ...
    def _get_full_path(self, fname, epoch: bool = False):
        if not epoch:
