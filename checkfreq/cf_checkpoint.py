...
class CFCheckpoint:
    ...
    If these objects hold CUDA tensors, then the snapshot holds
    ...
    def __init__(self, **kwargs):
        ...
        for name, ref in kwargs.items():
            if hasattr(ref, "state_dict"):
                ...
                self.logger.info(
                    f"Skipping object `{name}` in CF Checkpointing. \
				No state_dict() method exposed"
                )
    ...
    def _serialize_and_persist(
        self, filepath, snapshot, active_snapshot, lock, linkpath=None, iter_chk=None, epoch_chk=None, overwrite=True
    ):
        """
        ...
        update_stats(filepath, iter_chk=iter_chk, overwrite=overwrite, epoch_chk=epoch_chk, linkpath=linkpath)
